...
public abstract class CameraMixin {
    ...
    private void chroma$applyScreenshake(Camera instance, float yaw, float pitch, Operation<Void> original){
        ...
        boolean enabled = Objects.requireNonNull(GlobalConfigHandler.instance.getConfigForId(Chroma.MOD_ID)).get(
                Identifier.fromNamespaceAndPath(Chroma.MOD_ID, "flare_enabled")
        );
        float mult = Objects.requireNonNull(GlobalConfigHandler.instance.getConfigForId(Chroma.MOD_ID)).get(
                Identifier.fromNamespaceAndPath(Chroma.MOD_ID, "force_multiplier")
        );
